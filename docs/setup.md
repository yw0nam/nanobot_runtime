...
에서 해당 블록을 빼거나, 환경변수 (`YURI_TTS_ENABLED=0`, `YURI_IDLE_ENABLED=0`,
...
- **간단 케이스 (on/off, URL/타임아웃 변경)**: `.env` 의 `YURI_TTS_ENABLED`
  `YURI_IDLE_ENABLED` `YURI_LTM_URL` `YURI_TTS_URL` 등 환경변수만 수정.
